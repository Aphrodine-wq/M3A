...
export function FeaturedCompaniesSection() {
  ...
  return (
    <section id="companies" className="py-20 md:py-28 bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {featuredCompanies.map((company, index) => (
            <ScrollReveal key={company.id} delay={index * 100}>
              <Link href={`/portfolio/${company.slug}`}>
                <div className="group bg-white rounded-xl p-6 md:p-8 border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                  {/* Logo/Initials */}
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-14 h-14 bg-[#1A3C6E] rounded-lg flex items-center justify-center text-white font-bold text-xl">
                  ...
                  {/* Content */}
                  <h3 className="text-xl font-bold text-[#1A3C6E] mb-2 group-hover:text-[#3B82F6] transition-colors">
                    {company.name}
                  </h3>
        ...
        <ScrollReveal>
          <div className="text-center">
            <Link
              href="/portfolio"
              className="inline-flex items-center gap-2 px-8 py-4 bg-[#1A3C6E] text-white font-semibold rounded-full hover:bg-[#14325a] transition-all duration-300 hover:shadow-lg group"
            >
              View All Companies
